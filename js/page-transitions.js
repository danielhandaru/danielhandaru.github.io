$(function () {
  ...
  // Guard against null: if a page is missing .loading-text the original code threw
  // and halted smoothState bootstrap entirely (every link reverted to full reload).
  if (textWrapper) {
  ...
  // Flag toggled by onAfter to request the second keyframe. Lets us hold the
  // LOADING text visible during the actual transition (original Cygni intent)
  // without stranding it when the page loads faster than keyframe 1 finishes.
  var ptResumeRequested = false;
  ...
  var ptText = anime
    ...
    .add({
      ...
      complete: function (anim) {
        // Pause here ONLY if onAfter hasn't already asked for the resume.
        // If the page loaded fast and onAfter beat us to it (set the flag
        // while keyframe 1 was still running), let the timeline flow
        // straight into keyframe 2 instead of stranding LOADING visible.
        if (!ptResumeRequested) {
          ptText.pause();
        }
      },
    })
    .add({
      ...
      delay: (el, i) => 30 * i,
      complete: function (anim) {
        // Fix old removeClass('.page-change') typo — removeClass takes a
        // class name (no leading dot). The .page-change class was leaking.
  ...
  var options = {
      ...
      // prefetch:true + cacheLength:0 used to fetch every link on hover and then
      // throw it away. On slow connections this saturated the network and was
      // a major cause of "site is slow / sometimes doesn't load at all".
      prefetch: false,
      cacheLength: 5,
      onStart: {
        duration: 800, // Duration of our animation (was 2000 — felt sluggish)
        render: function ($container) {
          // Reset the keyframe-2 resume flag for this fresh navigation. If onAfter
          // beats keyframe 1 to the finish line, it will flip this back to true and
          // the pause inside keyframe 1's complete callback will become a no-op.
          ptResumeRequested = false;
      ...
      onReady: {
        ...
        render: function ($container, $newContent) {
          ...
          $(document).ready(function () {
            // Strip page-specific classes left over from the previous page
            // before re-applying the new page's classes. Without this they
            // accumulate (layout-light layout-dark, dark light, etc.) and
            // cause theme/colour glitches after a few navigations.
            $("body").removeClass(function (i, c) {
              return (c.match(/(^|\s)(layout-|header-style-)\S+/g) || []).join(
                " ",
              );
            });
            $(".site-header").removeClass(
              "dark light header-style-light header-style-dark dark-nav-active light-nav-active",
            );
            $(".site-navigation").removeClass("dark light classic");
            ...
            if ($(".has-animation").length > 0) {
              ...
              $(
                ".lines-up, .lines-down, .lines-fade-up, .lines-fade-down",
              ).each(function () {
                $(this).splitLines({
            ...
            if ($(".big-slider").length > 0) {
              ...
              $(".big-slider-item .title, .big-slider-item .summary").each(
                function () {
                  $(this).splitLines({
              ...
              var bigImages = new Swiper(".big-images", {
                ...
                on: {
                  ...
                  transitionEnd: function () {
                    ...
                    $(".swiper-pagination-bullet-active").removeClass(
                      "progress-init",
                    );

                    setTimeout(function () {
                      $(".swiper-pagination-bullet-active").addClass(
                        "progress-init",
                      );
                    }, 1);

                    var bsURL = $(".big-slider-item.big-item-active a").attr(
                      "href",
                    );
            ...
            if ($(".cygni-horizontal").length > 0) {
              ...
              $(".horizontal-item").each(function (i) {
                ...
                if ($(this).find("video").length > 0) {
                  ...
                  var horWrapper = $(
                    ".cygni-horizontal-images .swiper-wrapper",
                  );
              ...
              loadingAn.finished.then(function () {
                ...
                setTimeout(function () {
                  $(".cygni-horizontal-titles .title span").each(
                    function (i, element) {
                      $(element)
                        .delay(i * 175)
                        .queue(function (next) {
            ...
            if ($(".portfolio-grid").length) {
              ...
              $(
                ".grid-project-cat, .grid-project-title, .grid-project-index",
              ).each(function () {
                $(this).wrapInner("<span></span>");
              });
              ...
              var detailedImages = new Swiper(".detailed-images", {
                ...
                on: {
                  init: function () {
                    ...
                    $(
                      ".project-year, .project-category, .project-excerpt",
                    ).wrapInner("<span></span>");
      ...
      onAfter: function ($container, $newContent) {
        ...
        // subsequent ptText.play() resumes into keyframe 2.
        ptResumeRequested = true;

        if ($(".menu-toggle").hasClass("is-active")) {
          $(".menu-ov").removeClass("menu-ov-in");
          // Also reset the click-count state so the next toggle click goes
          // to the correct branch. Without this, the toggle state can desync
          // from .is-active and the menu sometimes "doesn't open" on next click.
        ...
        setTimeout(function () {
          // Destroy the previous instance so its scroll/resize listeners
          // don't accumulate on every navigation (was a memory + perf leak).
          if (
            window.peScroll &&
            typeof window.peScroll.destroy === "function"
          ) {
            try {
              window.peScroll.destroy();
            } catch (e) {}
          }
        ...
        if ($(".big-slider").length > 0) {
          ...
          setTimeout(function () {
            $(".bs-bullets .swiper-pagination-bullet").each(
              function (i, element) {
                $(element)
                  .delay(i * 200)
                  .queue(function (next) {
        ...
        // Re-bind scroll-triggered text highlights for the newly swapped #main
        // content. Delay matches the LocomotiveScroll re-init above so splitLines
        // has finished re-wrapping any .text-highlight spans before we observe them.
        if (typeof window.initTextHighlights === "function") {
          setTimeout(window.initTextHighlights, 1200);
        }
        ...
        if (typeof window.isIndexPage === "function" && window.isIndexPage()) {
          ...
          if (typeof window.dismountSpotifyPlayer === "function") {
            window.dismountSpotifyPlayer();
          }
        }
      },
    },
